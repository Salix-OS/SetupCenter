...
### This first section can be configured manually by any distribution integrating
### SetupCenter:
...
### SetupCenter, the following section shouldn't need further manual modification.
### From then on, it will be automatically managed by the Preferences dialog box.
...
# Syntaxe is => ['Category no',_('Name of the category'),'icon',bolean value for activation state],\
# There should be no unset categories! For the moment only 6 categories are supported.

# Set the utilities that should be in some (or all) of the categories:
cat1_list = ['gtkiconrefresh',]
cat2_list = ['gtkclocksetup','gtkkeyboardsetup','gparted','gtkalsasetup','ndisgtk','cups',]
cat3_list = ['gslapt','dotnew','sourcery',]
cat4_list = ['gtkhostsetup','gtkusersetup','gtkservicesetup','lilosetup','gtklocalesetup',]
cat5_list = ['persistence-wizard',]
cat6_list = ['',]
