...
class SetupCenter:
    ...
    def __init__(self):
        ...
        self.DisplayedCategoryListStore = builder.get_object(
            'displayed_category_liststore')
        self.AvailableCategoryListStore = builder.get_object(
            'available_category_liststore')
        self.ActivatedCategoryListStore = builder.get_object(
            'activated_category_liststore')
        self.CategoriesTreeview = builder.get_object(
            'categories_treeview')
        self.Category_Name_Column = builder.get_object(
            'category_name_column')
        self.Category_Icon_Column = builder.get_object(
        ...
        self.UtilitiesTreeViewColumn = builder.get_object(
            'utilities_treeviewcolumn')
        ...
        self.Cat5Label = builder.get_object('cat5_label')
        self.Cat6Label = builder.get_object('cat6_label')
        self.UtilityCellRendererCombo = builder.get_object(
            "utility_cellrenderercombo")
        self.CategoryCellRendererText = builder.get_object(
            "category_cellrenderertext")
        self.AboutDialog = builder.get_object("about_dialog")
        self.PreferencesDialog = builder.get_object("preferences_dialog")
        self.GlobalViewRadioButton = builder.get_object(
            "global_view_radiobutton")
        ...
        self.UtilityActivationToggle = builder.get_object(
            "utility_cellrenderertoggle")
        self.CategoryActivationToggle = builder.get_object(
            "category_cellrenderertoggle")
        self.PrefNulButton = builder.get_object("preferences_nul_button")
        self.PanedView = builder.get_object("paned_view")
        self.CategoryPanedView = builder.get_object(
...
### GUI INITIALIZATION ###
        ...
        self.AboutImage.set_from_icon_name(
            pref.icon_name,
            pref.about_icon_size
            )
        self.MainWindow.set_icon(
            get_icon(gtk.STOCK_OPEN, pref.icon_name, pref.title_icon_size)
            )
        self.AboutDialog.set_logo(
            get_icon(gtk.STOCK_OPEN, pref.icon_name, pref.about_icon_size)
            )
        self.MainWindow.set_title(
            pref.setupcenter_name
            )
        self.MainWindow.resize(
            pref.main_window_size[0],
            pref.main_window_size[1])
        self.MainWindow.set_position(gtk.WIN_POS_CENTER_ALWAYS)
        self.PanedView.set_position(0)

        # Set the utility combocell
        self.UtilityCellRendererCombo.set_property(
            "model",
            self.ActivatedCategoryListStore
            )
        ...
        self.UtilitiesTreeViewColumn.set_attributes(
            self.UtilityCellRendererCombo, text=2
            )

        # Set the category CellRendereText
        self.CategoryCellRendererText.set_property(
            'editable', True
            )
        self.CategoryCellRendererText.set_property(
            'cell_background', '#CCCCCC'
            )

        # Center the labels of the categories in the paned view
        self.CategoryPanedView.set_property(
            "alignment",
            pango.ALIGN_CENTER
            )
        self.CategoryPanedView.set_property(
        ...
        def set_applications(i):
            """
            Put each utility found in the user's system in its given category
            (set in preference file).
            """
                ...
                if proposed_application_name in utility_list:
                    ...
                    if show_cat == 'yes':
                        ...
                        utility_feedline = (
                            ...
                            pref.category_list[i][1],
                            'gtk-properties',
                            True
                            )
                        self.ApplicationListStore.append(utility_feedline)
                        #  Remove the application from the list to ensure
                        # it is only fed once
        ...
        def set_categories(i):
            ...
            try:
                ...
                get_value_list_from_liststore(
                    self.AvailableCategoryListStore,
                    0
                    )
                # Set the available category liststore
                # (there should be no doublon):
                ...
                try:
                    if state_feed:
                        ...
                        stores[i].get_iter(0)
                        self.DisplayedCategoryListStore.append(
                            displayed_category_feedline
                            )
                except ValueError:
                    pass
            except IndexError:
        ...
        def refresh_displayed_categories():
            """
            Previously hidden category boxes will have to be displayed if
            they now have content while newly emptied category boxes will
            have to be hidden.
            """
            self.DisplayedCategoryListStore.clear()
            for i in (0, 1, 2, 3, 4, 5):
                ...
                if pref.category_list[i][3]:
                    try:
                         # if it throws an exception we'll hide the category
                         # from global view
                        stores[i].get_iter(0)
                        global_view_cat[i].show()
                    except:
                        global_view_cat[i].hide()
            # Set the cursor on the first row and show
            # the first non-empty category
        ...
        def anulate_preferences():
            """
            Anulate any settings that was performed in the current
            preferences dialog box
            """
           # Restore the preference file
            shutil.move(
                pref_path + 'setupcenter_pref.old',
        ...
        def update_displayed_categories_labels(
            new_text,
            which_category,
            cat_number
            ):
        ...
        # Finalize categories/utilities (we can have application for up
        # to 6 categories)
        for i in (0, 1, 2, 3, 4, 5):
            set_applications(i)
        refresh_displayed_categories()

        # Feed the rest of the available applications
        # ' liststore (the ones with no assigned category)
        while utility_list:
            i = utility_list.pop()
            ...
            if i != 'setupcenter':
                 # Get the application human-readable name
                desktop_file = d.DesktopEntry(
                    filename='/usr/share/applications/' + i + '.desktop'
                    )
                human_readable = desktop_file.getName().split(',')[0]
                utility_feedline = (
                    human_readable, i, 'Select...', 'gtk-properties', False
                    )
...
### Catching signals from the GUI
        ...
        if self.AvailableCategoryListStore[path][3]:
            ...
            if liststore_content != []:
                ...
                new_feedline = [
                    toggled_category,
                    category_name,
                    self.AvailableCategoryListStore.get_value(iter, 2)
                    ]
    ...
    def on_category_cellrenderertext_edited(
        self,
        widget,
        path_string, new_text
        ):
        """
        Action to perform when a category name is edited in the
        ...
        if treeview_column.get_title() == _('Set the icon: '):
            ...
            try:
                icon_path = icon_theme.lookup_icon(
                    self.AvailableCategoryListStore[path][5],
                    48,
            ...
            new_icon_selection = gtk.FileChooserDialog(
                title=_('Choose another icon'),
                parent=None,
            ...
            new_icon_selection.connect(
                "update-preview", update_preview_cb, preview)
    ...
    def on_utility_cellrenderercombo_changed(
        self,
        widget,
        path_string,
        new_iter
        ...
        """
        Action to perform when a combo line is changed in the
    ...
    def on_iconview_item_activated(self, iconview, path):
        ...
        if 'gksu' in selected_action:
            ...
            # find out the user who executed setupcenter so it can
            # run the command:
            for line in commands.getoutput(
                "ps aux | grep 'su ' | grep setupcenter"
                ).splitlines():
                if 'root' not in line:
        ...
        if relevant_liststore.get_value(iter, 3) is True:
            utility_process = subprocess.Popen(
                'xterm ' + to_execute,
                shell=True
                )
